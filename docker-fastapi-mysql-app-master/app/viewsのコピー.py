...
@app.post("/register")
def create_user(username: str = Form(...), password: str = Form(...), family_name: str = Form(...)):
    """
    ユーザ登録をおこなう
    ...
    family_model = FamilyModel(config)
    family_model.create_family(family_name)
    family = family_model.find_family_by_family_name(family_name)
    family_id = family["id"]
    auth_model = AuthModel(config)
