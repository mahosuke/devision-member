...
class AuthModel(AbstractModel):
    ...
    def create_user(self, username, password, family_id, family_number):
        """
        新規ユーザ作成
        :param username: ユーザ名
        :param password: パスワード
        familynumberを末尾に追加した（17:49.6.11）
        :return:
        """
        hashed_password = self.hash_password(password)
        sql = "INSERT INTO users(username, password, family_id, family_number) VALUE (%s, %s ,%s, %s);"
        self.execute(sql, username, hashed_password, family_id, family_number)
